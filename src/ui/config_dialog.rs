...
impl ConfigDialog {
    ...
    pub fn get_feed(&self) -> String {
        /*
        if let Some(value) = &self.feed_tab.uri_entry.text() {
...
impl FeedTab {
    ...
    pub fn new() -> Self {
        ...
        let header_buffer = test_result.buffer();
    ...
    pub fn update_test_results(&self, result: Result<(), crate::state::ErrorType>) {
        if let Err(err) = result {
            ...
            if let Some(buffer) = self.test_result.buffer() {
    ...
    pub fn get_headers(&self) -> Vec<(String, String)> {
        ...
        for row in 0..rows {
            if let Some(iter) = self.list_store.iter_nth_child(None, row) {
                let field: String = self.list_store.value(&iter, 0).get().unwrap();
                let value: String = self.list_store.value(&iter, 1).get().unwrap();
...
impl FeedRequestMethod {
    ...
    pub fn get_request_method(&self) -> crate::config::RequestMethod {
        if let Some(value) = self.combo_box.active_id() {
    ...
    pub fn toggle_body_with_activation(&self, body_container: gtk::Box) {
        self.combo_box.connect_changed(move |combo_box| {
            if let Some(value) = combo_box.active_id() {
...
impl FeedRequestBody {
    pub fn new() -> Self {
        ...
        let header_buffer = request_body.buffer();
        if let Some(buffer) = header_buffer {
            buffer.set_text("");
        }
    ...
    fn set_body(&self, body: &Option<String>) {
        let header_buffer = self.request_body.buffer();
        if let Some(buffer) = header_buffer {
...
impl PollingInterval {
    pub fn new() -> Self {
        ...
        let info_message = gtk::Label::builder()
    ...
    pub fn get(&self) -> Result<u32, ()> {
        /*
        if let Some(text) = self.entry.text() {
